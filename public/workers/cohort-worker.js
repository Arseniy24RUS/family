/* Generated from src/core/cohort.js */
 ...
 */
const COHORT_VERSION='cohort-monthly/1.2.1';
...
function simulateCohort(d,options={}){
 ...
 for(let n=start;n<=end;n++){
  ...
  for(const[s,sexShare]of[['male',share],['female',1-share]]){
   ...
   if(net[s]<0&&d.migration_allocation==='stock_weighted_outflow'){
    const weights=normalized(next[s].map((v,j)=>v*mw[s][Math.min(Math.floor(j/12),100)]));
    for(let j=0;j<weights.length;j++){next[s][j]+=net[s]*weights[j];if(next[s][j]<0)throw Error(`${key}, ${s}: отток превышает доступную численность.`);}
    continue;
   }
