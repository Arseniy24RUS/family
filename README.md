...
*The application interface is Russian-only. The README is bilingual, but the screenshots and GIF show the real UI instead of invented translations.*
...
Run locally with Node 22+ and Python:
...
*Интерфейс приложения русскоязычный. README двуязычный, но изображения показывают реальный UI без выдуманного перевода.*
...
Соберите сайт командой `npm run build` и публикуйте каталог `docs/` через GitHub Pages. Пути относительные, маршруты используют hash, поэтому имя репозитория не зашито в код. Workflow в `.github` обновляет ЕМИСС и публикует default branch; расписание GitHub выполняется не строго по минутам и может отключаться у неактивных публичных репозиториев. После планового запуска проверяйте Actions logs и манифесты `docs/data/latest/`, прежде чем цитировать обновлённое значение.
