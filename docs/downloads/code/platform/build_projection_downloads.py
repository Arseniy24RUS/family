...
def run(root=ROOT):
    ...
    index=json.loads((root/'public/data/projections/indicators/manifest.json').read_text());rows=[];models=[]
    for e in index['series']:
        if not e.get('file'):continue
        obj=json.loads((root/'public'/e['file']).read_text())
    ...
    (out/'indicator_manifest.json').write_text(json.dumps(index,ensure_ascii=False,indent=2))
    (out/'population_manifest.json').write_bytes((root/'public/data/projections/population/manifest.json').read_bytes())
    files={f'demography/{fn}':(root/f'scripts/demography/{fn}').read_text() for fn in ['cohort.py','indicator.py','indicator_v11.py']};files['demography/__init__.py']='';files['reproduce_projection.py']=(root/'scripts/reproduce_projection.py').read_text()
    readme='Новые алгоритмы платформы 1.2.0. Нужен Python 3.11+ и NumPy.\npython -m pip install -r requirements.txt\npython reproduce_projection.py indicator forecast.json\npython reproduce_projection.py cohort cohort_package.json\n\nПример example_synthetic.json полностью синтетический; к России и регионам не относится.\n'
    ...
    (out/'projections_colab.ipynb').write_text(json.dumps(notebook,ensure_ascii=False,indent=2))
