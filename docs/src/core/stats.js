...
export function quantile(a, q) { let s = a.filter(finite).sort((a, b) => a - b); if (!s.length)
...
export function pearson(x, y) { if (x.length < 3 || x.length !== y.length)
...
export function ranks(xs) { const idx = xs.map((v, i) => ({ v, i })).sort((a, b) => a.v - b.v), out = []; for (let i = 0; i < idx.length;) {
    let j = i;
...
export const hhi = values => { let sum = values.reduce((s, x) => s + x, 0); return sum > 0 ? values.reduce((s, x) => s + (x / sum) ** 2, 0) : null; };
export const did = (beforeTreatment, afterTreatment, beforeControl, afterControl) => (afterTreatment - beforeTreatment) - (afterControl - beforeControl);
export const rng = seed => () => { let t = seed += 0x6D2B79F5; t = Math.imul(t ^ t >>> 15, t | 1); t ^= t + Math.imul(t ^ t >>> 7, t | 61); return ((t ^ t >>> 14) >>> 0) / 4294967296; };
export function standardize(matrix) { const p = matrix[0].length; let means = Array.from({ length: p }, (_, j) => mean(matrix.map(r => r[j]))), std = Array.from({ length: p }, (_, j) => Math.sqrt(mean(matrix.map(r => (r[j] - means[j]) ** 2))) || 1); return { z: matrix.map(r => r.map((v, j) => (v - means[j]) / std[j])), means, std }; }
export function kmeans(matrix, k = 3, seed = 42) {
...
export function moran(values, adjacency, permutations = 999, seed = 42) { const ids = Object.keys(values).filter(k => finite(values[k])), n = ids.length; if (n < 5)
