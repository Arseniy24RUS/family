...
set -euo pipefail
runtime_root="${SEMYA_RUN_DIR:-${RUNNER_TEMP:?}}"
private="$runtime_root/semya-private"
collector="semya"
interface="tun0"
...
check_route() {
  ...
  # Validate representative routes, including both official EMISS hostnames.
  for host in www.fedstat.ru fedstat.ru; do
    local address
...
case "${1:-}" in
  start)
    test -n "${EMISS_OPENVPN_CONFIG_B64:-}" || { echo 'Home VPN secret is missing.'; exit 1; }
    umask 077
    mkdir -p "$private"
    chmod 700 "$private"
    printf '%s' "$EMISS_OPENVPN_CONFIG_B64" | base64 --decode > "$private/home.ovpn"
    ...
    sudo iptables -N SEMYA_VPN
    sudo iptables -A SEMYA_VPN -o lo -j ACCEPT
    sudo iptables -A SEMYA_VPN -o "$interface" -j ACCEPT
    sudo iptables -A SEMYA_VPN -j REJECT
    sudo iptables -I OUTPUT 1 -m owner --uid-owner "$collector" -j SEMYA_VPN
    ...
    sudo iptables -N SEMYA_DNS
    sudo iptables -A SEMYA_DNS -o lo -j RETURN
    sudo iptables -A SEMYA_DNS -o "$interface" -j RETURN
    sudo iptables -A SEMYA_DNS -j REJECT
    sudo iptables -I OUTPUT 1 -p udp --dport 53 -j SEMYA_DNS
    sudo iptables -I OUTPUT 1 -p tcp --dport 53 -j SEMYA_DNS
  ...
  test-isolation)
    ...
    sudo iptables -I SEMYA_VPN 1 -o "$interface" -j REJECT
    trap 'sudo iptables -D SEMYA_VPN -o "$interface" -j REJECT' EXIT
  ...
  stop)
    ...
    if id "$collector" >/dev/null 2>&1; then
      sudo iptables -D OUTPUT -m owner --uid-owner "$collector" -j SEMYA_VPN 2>/dev/null || true
      sudo ip6tables -D OUTPUT -m owner --uid-owner "$collector" -j REJECT 2>/dev/null || true
    fi
    sudo iptables -F SEMYA_VPN 2>/dev/null || true
    sudo iptables -X SEMYA_VPN 2>/dev/null || true
    sudo iptables -D OUTPUT -p udp --dport 53 -j SEMYA_DNS 2>/dev/null || true
    sudo iptables -D OUTPUT -p tcp --dport 53 -j SEMYA_DNS 2>/dev/null || true
    sudo iptables -F SEMYA_DNS 2>/dev/null || true
    sudo iptables -X SEMYA_DNS 2>/dev/null || true
    ...
    test "$private" = "$runtime_root/semya-private" && sudo rm -rf -- "$private"
    ...
    sudo rm -rf -- "$runtime_root/semya-runtime"
